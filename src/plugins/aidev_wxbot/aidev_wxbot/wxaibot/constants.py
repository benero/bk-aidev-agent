...
# 收尾时排空 Agent 流的上限（秒）。取消后 producer 通常一两秒内就写出 EOD，
# 给足余量即可；超时只是放弃等 EOD，不影响后续清理。
...
TOOL_LINE_PREFIX = "> "
# 只展示首个参数值作为「操作对象」，罗列全部参数会让工具块比正文还长
TOOL_TARGET_LIMIT = 50
# 成功的结果由正文复述，无需重复；只有失败必须让用户看到原因
TOOL_ERROR_LIMIT = 200
